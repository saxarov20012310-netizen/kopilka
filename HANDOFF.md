...
- **АВТОСИНХРОНИЗАЦИЯ (июль 2026):** эндпоинт отдаёт ещё `monthShifts` (id/date/tips за текущий
  календарный месяц) и `monthTips`. В копилке StoreProvider после сверки с облаком раз за сессию
  сам тянет skazka (getTelegramUserId) и: обновляет ритм (SET_SETTINGS shiftsPerMonth/tipsPerShift),
  импортирует смены месяца (action IMPORT_SHIFTS) — по каждой новой смене создаёт income-операцию
  category 'tips' на долю чая (tips×savingRate), помечает `Transaction.skazkaId` для идемпотентности
  (одна смена не задвоится). Кнопка в Настройках теперь «Обновить из skazka сейчас» (ручной триггер
  того же, авто — основной путь). Тест логики: scratchpad/t.mjs (720+1280+440=2440 по 3 сменам июля).
