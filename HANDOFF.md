...
## ✅ РЕДИЗАЙН «AURORA» — ТЕКУЩИЙ (Revolut-премиум, июль 2026)
Поверх «Электрик» наведён премиум-финтех-вид. Единая ТЁМНАЯ айдентика (не зависит от темы Telegram —
...
Прежняя секция «Электрик» ниже — историческая (палитра индиго/лайм), актуальна Aurora.

## Историческое: РЕДИЗАЙН «ЭЛЕКТРИК» (июль 2026)
...
- **ЗАРАБОТОК ОТДЕЛЬНО ОТ КОПИЛКИ (июль 2026, важно!):** данные из skazka — это ЗАРАБОТОК,
  а НЕ отложенное (пользователь не весь чай откладывает). AppState.skazka: SkazkaSnapshot
  (fetchedAt, monthShifts, monthTips, avgTips, shiftsPerMonth). Автосинк (StoreProvider, раз/сессию,
  после сверки с облаком) обновляет ритм в settings + кладёт снимок через SET_SKAZKA.
  Старый автоимпорт «отложено по смене» УДАЛЁН; миграция в storage.mergeDefaults вычищает
  транзакции с skazkaId. Вкладка История — сегмент «Копилка | Заработок»: заработок месяца
  по кассовому принципу (calcMonthEarnings в calc.ts): чай смен + зарплата 5-го (ЗА ПРОШЛЫЙ месяц,
  подписывается «за июнь») + аванс 20-го; зарплата за текущий месяц — «придёт 5-го следующего»
  в блоке «ожидается». paydayInMonth в date.ts (кламп+сдвиг на пред. рабочий).
- **Совет (Motivation):** спокойная карта bg-surface без цветной заливки/окантовки («зелёная с белой
  окантовкой резала глаз»), внутри: статус к плану + «получено N, отложено M (Y%), нужно ≈X% —
  доложите K». Строка вердикта стратегии с Главной убрана (жила под плитками, дублировала).
  Падежи месяцев: monthNameNom/monthNamePrep в date.ts.
- Анимации: .rise (fade-up со stagger через animationDelay) на секциях Главной и Заработка,
  prefers-reduced-motion отключает.
- **Блок накоплений на Главной** (переделан по просьбе пользователя): фирменная тёмная карта
  bg-inverse — кольцо 112px слева (только % лаймом, ProgressRing принял props color/track для
  фиксированных цветов на тёмном), справа «Накоплено / сумма / из цели», ниже строками
  «Осталось накопить» (лайм) и «До дедлайна». Плитки Цель/Осталось и белая карта с кольцом удалены.
- Математика проверена: `npx vite-node scratch/math-test.ts` (сдвиг зарплаты 5.07→3.07 с воскресенья,
  received/expected/topUp/spent/free — все ассерты).
- **ОДИН ТАП «ОТЛОЖИТЬ» (июль 2026):** App экспортирует тип `OpenAdd = (kind, prefill?) => void` и
  `AddPrefill {amount?, category?, note?}`; AddTransaction принимает initialAmount/Category/Note.
  На Главной строки «Ближайшие поступления» — кнопки: тап открывает добавление с подставленной
  суммой (e.suggested), категорией (e.source) и заметкой. В «Совете» (Motivation, принимает onAdd)
  кнопка «Отложить N ₽» на topUp с category 'other'. Мигания нет, ADD_TX как обычно.
- **РАСХОДЫ КАК БЮДЖЕТ:** calcMonthEarnings += spentThisMonth (все расходы месяца) и
  free (получено − отложено − потрачено, может быть <0). В История→Заработок карта итога —
  3 колонки Отложено/Потрачено/Свободно (свободно краснеет при <0).
- **«ЧИСЛА ОЖИВАЮТ» (июль 2026):**
  · Честный прогноз срока — Strategy += monthlyAchievable/realisticDate (addMonths в date.ts);
    на Главной плашка при verdict tight/unreal: «Реальный срок при норме X% — <месяц год>, а не <дедлайн>».
  · Бюджет-подсказка в Заработке: «осталось N дн месяца → ≈M/день», при free<0 — красное предупреждение.
  · Празднование вех — AppState.celebratedPct (0/25/50/75/100), компонент Celebration (haptic+искры,
    авто-гаснет 2.2с), триггер в App при пересечении tier. Кейфреймы celebratePop/spark в index.css.
- **РЕЗЕРВНАЯ КОПИЯ:** storage.exportState/importState (обёртка {v,at,state} или голый AppState,
  валидация goal.target). Настройки → «Сохранить копию» (в буфер) / «Восстановить» (prompt→HYDRATE).
- **Бэклог в GitHub Issues** (saxarov20012310-netizen/kopilka): #1 пуши через бота (backend-эпик,
  НЕ начат — нужен процесс бота + хранилище профилей уведомлений + планировщик, server.js сейчас
  только статика, данные все на клиенте), #2 несколько целей, #3 перф скриншота превью.
